...
export default function AllBookingsTab({
  ...
  React.useEffect(() => {
    if (allBookingsFilter && allBookingsFilter !== 'all') {
      ...
      // Navigate to most recent month that has bookings for this type
  ...
  React.useEffect(() => {
    // Use a timeout to let filtered/pastFiltered recompute before reading them
    const id = setTimeout(() => {
      setShowPast(prev => {
        ...
        return upcomingCount === 0 && pastCount > 0
      })
    }, 0)

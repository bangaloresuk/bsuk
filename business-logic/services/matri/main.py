...
async def create(payload: SatsangCreate):
    ...
    try:
        async with get_session() as session:
            # Matri-Sammelan is single-venue — same date+time slot can't be
            ...
            if existing.scalars().first() is not None:
                return err(
                    f"Slot Already Booked! Matri-Sammelan on {payload.date} at "
            ...
            await session.flush()
            new_id = event.id
        return {"success": True, "id": str(new_id), "message": f"Matri-Sammelan booked! ID: {new_id}"}
    except Exception as e:
        raise HTTPException(status_code=502, detail=str(e))
